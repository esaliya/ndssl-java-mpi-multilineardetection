...
if [ $procbind = "core" ]; then
  $BUILD/bin/mpirun --verbose --hostfile $nodefile -mca btl_openib_ib_min_rnr_timer 25 -mca btl_openib_ib_timeout 20  --mca btl ^tcp --report-bindings --map-by ppr:$ppn:node:PE=$pe  --bind-to core --rank-by core  -np $(($nodes*$ppn)) java $opts -cp $cp org.saliya.ndssl.multilinearscan.mpi.Program -v $v -k $k -i $i -e $e -nc $nodes 2>&1 | tee $name.v$v.k$k.e$e.$pat.out.txt
elif [ $procbind = "socket" ]; then
  $BUILD/bin/mpirun --verbose --hostfile $nodefile -mca btl_openib_ib_min_rnr_timer 25 -mca btl_openib_ib_timeout 20  --mca btl ^tcp --report-bindings --map-by ppr:$ppn:node  --bind-to socket --rank-by core  -np $(($nodes*$ppn)) java $opts -cp $cp org.saliya.ndssl.multilinearscan.mpi.Program -v $v -k $k -i $i -e $e -nc $nodes 2>&1 | tee $name.v$v.k$k.e$e.$pat.out.txt
