...
cp=$HOME/.m2/repository/org/saliya/ndssl/multilineardetection/dag-1.0-SNAPSHOT/multilineardetection-dag-1.0-SNAPSHOT-jar-with-dependencies.jar:$HOME/sali/software/slf4j-1.7.25/slf4j-api-1.7.25.jar:$HOME/sali/software/slf4j-1.7.25/slf4j-nop-1.7.25.jar
...
if [ $procbind = "core" ]; then
  ...
  ~/sali/software/builds/ompi.2.0.2/bin/mpirun --mca btl ^openib --mca mtl ^psm --report-bindings --map-by ppr:$pps:socket:PE=$pe  --bind-to core:overload-allowed --rank-by core  -np $(($nodes*$ppn)) java $opts -cp $cp org.saliya.ndssl.multilinearscan.mpi.Program -v $v -i $i -nc $nodes -mms $mms -p $parts -tc $tpp -cps $cps -bind $bindThreads 2>&1 | tee $name.v$v.k$k.e$e.$pat.$partMethod.out.txt
else
